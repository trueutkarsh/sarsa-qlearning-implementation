...
try:
    ...
    print("Total Reward ", sum(episodeRewards))
    print("Average Reward ", sum(episodeRewards)*1.0/len(episodeRewards))
    print("Max reward ", max(episodeRewards))
    print("Positive reward Count ", sum(i > 0 for i in episodeRewards))
    print('Completed {} episodes.'.format(episodeNum))
    #k = 20
    #print("Last " + str(k) + " average rewards ",  sum(episodeRewards[-1*k:])*1.0/k)
finally:
    connection.close()
